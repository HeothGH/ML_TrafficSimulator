...
public class CameraController : MonoBehaviour
{
    ...
    void CreateSelectionRing()
    {
        ...
        // Dodajemy LineRenderer
        ringLineRenderer = selectionRingObject.AddComponent<LineRenderer>();

        // Konfiguracja wygl¹du
        ringLineRenderer.useWorldSpace = false; // Bêdzie porusza³ siê razem z obiektem
        ringLineRenderer.loop = true;           // Zamkniêty okr¹g
        ringLineRenderer.positionCount = segments;
        ringLineRenderer.startWidth = lineWidth;
        ringLineRenderer.endWidth = lineWidth;

        // Ustawienie prostego materia³u (Sprites/Default jest zawsze dostêpny i lekki)
    ...
    void UpdateSelectionRing()
    {
        if (targetToFollow != null)
        {
            ...
            // Przesuwamy pierœcieñ w miejsce samochodu
            // U¿ywamy pozycji auta, ale zerujemy rotacjê, ¿eby pierœcieñ siê nie krêci³ razem z autem
